...
String findWords(response, green, yellow, grey, wordNum) {
  ...
  int counter = 0;
  for (int wordLoc = 0;
      wordLoc < words.length && counter < wordNum;
      wordLoc++) {
    ...
    for (int i = 0; i < yellow.length; i++) {
      ...
      for (int l = 0; l < 5; l++) {
        if (word[l] == yellow[i][0]) fullCheck = true;
      }
      if (!fullCheck) compCheck = false;
    }
              ...
              InkWell(
                onTap: () async {
                  ...
                  if (greyController.text != "")
                    grey = prepareFormText(greyController.text).split(',');
                  if (yellowController.text != "")
                    yellow = prepareFormText(yellowController.text).split(',');
                  if (greenController.text != "")
                    green = prepareFormText(greenController.text).split(',');
                  ...
                  String output =
                      findWords(response, green, yellow, grey, wordNum);
                  setState(() {
                    log = output;
                  });
